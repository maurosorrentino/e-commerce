  ...
  fetchData = () => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/api/login`, {
      ...
      .then((resData) => {
        ...
        if (
          resData.message !== "invalid password, please try again" &&
          resData.message !==
            `There is no account into our database with this email: ${this.state.email}` &&
          resData.message !==
            "Successful Login, You Are Being Redirected To Our Shop" &&
          resData.message !==
          ...
          <MessageStyles>
            <h1
              className={
                this.state.message ===
                  `There is no account into our database with this email: ${this.state.email}` ||
                this.state.message === "invalid password, please try again" ||
                this.state.message === "Please Verify Your Account"
                  ? "red"
                  : ""
              }
              id="message-test"
        ...
        <Form id="login-form-test" onSubmit={this.loginHandler}>
          ...
          >
            ...
            <button id="button-test">
              Log{this.state.loading ? "ging" : ""} in!
            </button>
