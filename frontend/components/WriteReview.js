...
class WriteReview extends Component {
  ...
  fetchData = () => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/auth/api/write-review/${itemId}`,
      {
        method: "PUT",
          ...
          {this.state.message && (
            <MessageStyles>
              <h1
                className={
                  this.state.message ===
                    "you can only make 1 review per product" ||
                  this.state.message ===
                    "Only Users That Bought This Item Can Make A Review." ||
                  this.state.message ===
                    "You cannot take this action, please login"
                    ? "red"
                    : ""
                }
              >
