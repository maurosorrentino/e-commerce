...
class ItemAvailableAgain extends Component {
  ...
  fetchDataLoggedOut = () => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/email-me-item-out/${itemId}`,
      {
        method: "PUT",
    ...
    return (
      <>
        {cookie.load("authCookie") && (
          <button onClick={this.fetchDataLoggedIn}>
            Email Me When It Will Be Available Again!
          </button>
        )}
        ...
        {this.state.message === "please enter your email" && (
          <>
            ...
            <button onClick={this.fetchDataLoggedOut} id="button-logged-out">
              Send Me An Email
            </button>
          </>
        )}
