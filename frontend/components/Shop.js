...
class Shop extends Component {
            ...
            return (
              <>
                <StyleOfItem key={item._id}>
                  ...
                  {inStock === 0 && <ItemAvailableAgain itemId={item._id} />}
                </StyleOfItem>
              </>
            );
          }),
  ...
  render() {
    ...
    for (
      let i = 1;
      i <= Math.ceil(this.state.totalItems / this.state.perPage);
      i++
    ) {
      pageNumbers.push(i);
    }
    ...
    const renderPageNumbers = pageNumbers.map((pageNumber) => {
      return (
        <li
          ...
          id={pageNumber}
          className={
            this.state.currentPage === pageNumber ? "current-page" : ""
          }
        ...
        {!this.state.loading && this.state.items.length > 4 && (
          <Pagination>
            <ul>
              ...
              <button
                disabled={
                  this.state.currentPage === this.state.lastPage ? true : false
                }
