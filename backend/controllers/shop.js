  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({
          message: `We Couldn't Find An Account With The Following Email: ${email}`,
        });
    }
    ...
    return res
      .status(200)
      .json({ message: "Password Reset Requested, Please Check Your Email" });
  } catch (err) {
    console.log(err);
...
exports.getReviewStats = async (req, res) => {
  try {
    ...
    const averageReviews =
      (star1 + stars2 + stars3 + stars4 + stars5) / totalReviews;
...
exports.itemAvailableAgainLoggedIn = async (req, res) => {
  try {
    ...
    const alreadyExists = await ItemAvailableAgainUser.findOne({
      itemId,
    ...
    if (alreadyExists) {
      return res
        ...
        .json({
          message:
            "you already requested a follow up email notification for this item",
        });
    }
    ...
    return res
      ...
      .json({
        message:
          "Thank You, We Will Email You As Soon As We Have News On This Item",
        email: user.email,
      });
  } catch (err) {
    console.log(err);
