...
exports.signup = async (req, res) => {
  try {
    ...
    return res
      ...
      .json({
        message:
          "user created, please check your email in order to verify your account within 1 hour",
        user: savedUser,
      });
  } catch (err) {
    console.log(err);
...
exports.verifySignup = async (req, res) => {
  try {
    ...
    if (!tokenIsValid || Date.now() > user.tokenVerifyEmailExpires) {
      ...
      return res
        .status(401)
        .json({ message: "Sorry, Something Went wrong. Please Signup Again" });
    }
  ...
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({
          message: `There is no account into our database with this email: ${email}`,
        });
    }
    ...
    return res
      .status(200)
      .json({
        message: "Successful Login, You Are Being Redirected To Our Shop",
      });
  } catch (err) {
    console.log(err);
...
exports.createItem = async (req, res) => {
  try {
    if (!req.session.isAuth) {
      return res
        .status(401)
        .json({ message: "You cannot take this action, please login" });
    }
    ...
    if (title.length < 3) {
      return res
        .status(422)
        .json({ message: "Title needs to be at least 3 characters" });
    }
    ...
    if (price <= 0) {
      return res
        .status(422)
        .json({ message: "Price cannot be less or equal than 0" });
    }
    ...
    if (description.length < 5) {
      return res
        .status(422)
        .json({ message: "Description needs to be at least 5 characters" });
    }
    ...
    if (Number(stock) <= 0) {
      return res
        .status(422)
        .json({ message: "Items in stock need to be grater than 0" });
    }

    if (!Number.isInteger(Number(stock))) {
      return res
        .status(422)
        .json({ message: "the number has to be an integer" });
    }
    ...
    return res
      .status(200)
      .json({ message: `item ${title} was created`, item: itemSave });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (itemIndex >= 0) {
      ...
      if (cart[itemIndex].quantity > item.stock) {
        return res
          .status(401)
          .json({ message: `we only have ${item.stock} left into the store` });
      }
      ...
      res
        .status(200)
        .json({
          message: `${item.title} was added again to the cart. quantity: x ${itemQuantity}`,
        });
    ...
    return res
      .status(200)
      .json({ message: `Item ${item.title} was added to the cart` });
  } catch (err) {
    console.log(err);
...
exports.cartPage = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "fetched items", items, email, total });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (itemIndex === -1) {
      return res
        .status(401)
        .json({
          message: "Only Users That Bought This Item Can Make A Review.",
        });
    }
    ...
    if (duplicate) {
      return res
        .status(401)
        .json({ message: "you can only make 1 review per product" });
    }
    ...
    res
      .status(200)
      .json({ message: `Thank You ${name} For The Review`, userId });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (!isValid) {
      return res
        ...
        .json({
          message:
            "password is wrong, please confirm your password in order to change your details",
        });
    }
    ...
    return res
      .status(200)
      .json({ message: "you successfully changed your details" });
  } catch (err) {
    console.log(err);
...
exports.saveNewIban = async (req, res) => {
  try {
    ...
    if (iban.length > 34) {
      return res
        .status(422)
        .json({ message: "something is wrong with your iban" });
    }
